...
class PhotonIDValueMapProducer : public edm::stream::EDProducer<> {
  ...
  template <class T, class U>
  float computeWorstPFChargedIsolationWithPVconstrained(const T& photon,
  ...
  constexpr static char phoWorstChargedIsolationWithPVconstrained_[] = "phoWorstChargedIsolationWithPVconstrained";
  constexpr static char phoWorstChargedIsolationWithConeVetoWithPVconstrained_[] = "phoWorstChargedIsolationWithConeVetoWithPVconstrained";
...
constexpr char PhotonIDValueMapProducer::phoWorstChargedIsolationWithPVconstrained_[];
constexpr char PhotonIDValueMapProducer::phoWorstChargedIsolationWithConeVetoWithPVconstrained_[];
...
PhotonIDValueMapProducer::PhotonIDValueMapProducer(const edm::ParameterSet& iConfig) {
  ...
  produces<edm::ValueMap<float> >(phoWorstChargedIsolationWithPVconstrained_);  
  produces<edm::ValueMap<float> >(phoWorstChargedIsolationWithConeVetoWithPVconstrained_);  
...
void PhotonIDValueMapProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  std::vector<float> phoWorstChargedIsolationWithPVconstrained;
  std::vector<float> phoWorstChargedIsolationWithConeVetoWithPVconstrained;
  ...
  for (unsigned idxpho = 0; idxpho < src->size(); ++idxpho) {
    ...
    float worstChargedIsoWithPVconstrained =
      computeWorstPFChargedIsolationWithPVconstrained(iPho, pfCandidatesHandle, vertices, 
				     isAOD, pv,coneSizeDR, dxyMax, dzMax,
				     dRvetoBarrel, dRvetoEndcap, ptMin);
    phoWorstChargedIsolationWithPVconstrained .push_back( worstChargedIsoWithPVconstrained );
    ...
    dRvetoEndcap = 0.02;
    ptMin = 0.1;
    float worstChargedIsoWithConeVetoWithPVconstrained =
      computeWorstPFChargedIsolationWithPVconstrained(iPho, pfCandidatesHandle, vertices, 
				     isAOD, pv, coneSizeDR, dxyMax, dzMax,
				     dRvetoBarrel, dRvetoEndcap, ptMin);
    phoWorstChargedIsolationWithConeVetoWithPVconstrained .push_back( worstChargedIsoWithConeVetoWithPVconstrained );
  ...
  writeValueMap(iEvent, src, phoWorstChargedIsolationWithPVconstrained, phoWorstChargedIsolationWithPVconstrained_);  
  writeValueMap(iEvent, src, phoWorstChargedIsolationWithConeVetoWithPVconstrained, phoWorstChargedIsolationWithConeVetoWithPVconstrained_);  
  //PFCluster  Isolation
...
// Charged isolation with respect to the worst vertex. See more
// comments above at the function declaration. Here the impact parameter is constrained to belong from the PV
template <class T, class U>
float PhotonIDValueMapProducer
::computeWorstPFChargedIsolationWithPVconstrained(const T& photon, const U& pfCandidates,
				 const edm::Handle<reco::VertexCollection> vertices,
				 bool isAOD,const reco::Vertex& pv,
				 float dRmax, float dxyMax, float dzMax,
				 float dRvetoBarrel, float dRvetoEndcap, float ptMin){
